...
def parse_args(default_params_file = 'parameters/params.yml'): 
    #customized version of original "parse_args" function
...
def prepare_experiment_custom(name, args):
    #customized version of original "prepare_experiments" function
    if args.resume_from is None: 
        params_file = args.params_file 
        if not args.no_save: 
            current_time = datetime.datetime.now().strftime('%b%d_%H-%M-%S') 
            log_dir = os.path.join('results/{0}/'.format(name), 
                                   args.save_dir, 
                                   current_time) 
            checkpoint_dir = os.path.join(log_dir, 'checkpoints')
            output_dir = os.path.join(log_dir, 'output')
            tensorbrd_dir = os.path.join(log_dir, 'logs')
    ...
    if not args.no_save:  
        directories = {'log_dir':log_dir, 
                       'checkpoint_dir': checkpoint_dir,
                       'output_dir': output_dir,
                       'tensorbrd_dir': tensorbrd_dir} 
...
def prepare_experiment(name, args): 
    ...
    with open(params_file, 'r') as f: 
        import yaml 
        # params = yaml.load(f)         #yaml.load(f) is DEPRECATED !
...
def train(gen_train, decolle_loss, net, opt, epoch, burnin, online_update=True, batches_per_epoch=-1): 
    ...
    #activity rate = mean spike activity / ms 
    #in order to convert it to Firing Rate in Hz, just multiply by 1000!
    ...
    # total_loss /= t_sample
    # print('Training Loss {0}'.format(total_loss)) #loss per ms
    # print('Activity Rate {0}\n'.format(act_rate)) #mean spike activity/ms
...
def test(gen_test, decolle_loss, net, burnin, print_error = True, debug = False): 
    ...
    with torch.no_grad(): 
        ...
        test_loss /= len(gen_test)   #total_loss / num_batches = loss_per_batch
        print('Test Loss     {0}'.format(test_loss))  #loss per batch
...
def save_checkpoint(epoch, checkpoint_dir, net, opt): 
    ...
    save_path = os.path.join(checkpoint_dir,'epoch{:05}.tar'.format(epoch))
    torch.save({ 
        'epoch'               : epoch, 
